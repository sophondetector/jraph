WITH traverse(id, relation, hops) AS (
  SELECT node_id, CAST(node_id AS VARCHAR), 0 FROM node WHERE json_value(properties, '$.Name')='Jon'
  UNION ALL
...
FROM traverse
WHERE id = (SELECT node_id FROM node WHERE json_value(properties, '$.Name')='Nate');
