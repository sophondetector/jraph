...
* `JROOT/*` contains all the container system and build files; the directory is essentially part of the `jraph` image schema.
* `JROOT/root/` contains all provisioning scripts `prov-*.sh`, which are all packaged together into `/root/entrypoint.sh`.
